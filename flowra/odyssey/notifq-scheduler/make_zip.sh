...
# (Excludes local helpers like validate*.sh / DRAFT.md / make_zip.sh by only
#  adding the 5 named paths, and strips caches / grader artifacts.)
#
# Usage:  bash odyssey/notifq-scheduler/make_zip.sh
set -euo pipefail

BASE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$BASE"
OUT="$BASE/../notifq-scheduler.zip"
